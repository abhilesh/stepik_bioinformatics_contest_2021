...
def annotate_metabolites(m_masses, k_masses, n_masses):
    ...
    for n_id in n_masses_ids:
        ...
        m_k_pair = closest_metabolite_adduct_pair(
            sorted_m_masses, sorted_k_masses, n_mass
        )
...
def process_metabolite_signal(input_file, output_file):

    fin_lines = open(input_file, "r").readlines()
    fout_file = open(output_file, "w")
...
if __name__ == "__main__":
    ...
    test_files = input_dir.glob("*.txt")

    for test in test_files:
        print(f"Working on file: {test.name}")
        fin_file = Path(input_dir / test)
        fout_file = Path(output_dir / test.name).with_suffix(".out")
        process_metabolite_signal(input_file=fin_file, output_file=fout_file)
    ...
    print(
        f'Running time: {time.strftime("%H:%M:%S", time.gmtime(end_time - start_time))}'
    )
